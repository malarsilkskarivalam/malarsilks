...
export default function AdminPage() {
  ...
  const fetchOrders = async (isPolling = false) => {
    try {
      ...
      const data = await res.json()
      if (data.success) {
        const fetched = data.data.map((o: any) => ({ ...o, id: o._id || o.id }))
        setOrders(fetched)

        if (isPolling) {
          // Compute new unseen pending orders outside of setState
          const pendingOrders = fetched.filter((o: any) => o.status === 'Pending')
          const newUnseen = pendingOrders.filter((o: any) => !seenOrderIds.has(String(o.id)))

          // Call toast outside of setState to avoid React warning
          if (newUnseen.length > 0 && activeTab !== 'orders') {
            toast({
              title: `🛒 ${newUnseen.length} New Order${newUnseen.length > 1 ? 's' : ''}!`,
              description: `You have new pending order${newUnseen.length > 1 ? 's' : ''} waiting for review.`,
            })
          }
          ...
          // Initial load — mark all current pending as seen so we don't alert on page load
          const pendingIds = fetched
            .filter((o: any) => o.status === 'Pending')
            .map((o: any) => String(o.id))
          setSeenOrderIds(new Set(pendingIds))
        }
      }
    } catch (error) {
  ...
  const handleDeleteCustomer = async (id: string) => {
    if (!confirm('Are you sure you want to delete this customer? This will also affect their access.')) return
    try {
      const token = localStorage.getItem('adminToken')
      const res = await fetch(`${CONFIG.API.BASE_URL}/api/auth/users/${id}`, { 
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      })
      const data = await res.json()
  ...
  // Auto-refresh orders in the admin panel every 10 seconds
  useEffect(() => {
    if (isAuthenticated) {
      // Initial fetch (not polling - just marks existing as seen)
      fetchOrders(false)
      
      const interval = setInterval(() => {
        fetchOrders(true) // polling = true, so it alerts on new orders
      }, 10000)
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Tabs defaultValue="dashboard" className="space-y-6" onValueChange={(val) => {
            setActiveTab(val)
            if (val === 'orders') {
              // Mark all pending orders as seen when admin opens Orders tab
          ...
          <TabsList className="grid w-full grid-cols-6 border-b pb-0">
            ...
            <TabsTrigger value="orders" className="relative">
              Orders
              {newOrdersCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1 animate-bounce shadow-lg">
                  {newOrdersCount}
                </span>
              )}
            </TabsTrigger>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onFocus={(e) => { if (newProduct.price === 0) e.target.value = '' }}
                ...
                <div className="space-y-4">
                  {orders.length === 0 ? (
                    ...
                    orders.map((order: any, idx: number) => {
                      ...
                      const sequentialNumber = orders.length - idx;

                      return (
                        <div key={order.id || order._id} className={`p-4 border rounded-xl hover:bg-accent/5 transition-all duration-300 relative overflow-hidden ${
                          order.status === 'Cancelled' ? 'opacity-60' : 
                          order.status === 'Delivered' ? 'bg-green-50/50 border-green-200' : 
                          ''
                        }`}>
                          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div 
                              className={`cursor-pointer flex-1 ${order.status === 'Cancelled' ? 'line-through decoration-gray-500 decoration-2' : ''}`}
                              onClick={() => setExpandedOrderId(isExpanded ? null : (order.id || order._id))}
                            >
                              <div className="flex items-center gap-2">
                                <p className="font-bold text-lg">Order #{sequentialNumber}</p>
                                <span className="text-xs text-muted-foreground bg-gray-100 px-2 py-0.5 rounded">
                                  ID: {String(order.id || order._id || '').substring(0, 8).toUpperCase()}
                                </span>
                                <span className="text-[10px] text-gray-400 font-semibold ml-2">
                                  {isExpanded ? '(Collapse ⬆️)' : '(Expand for details ⬇️)'}
                                </span>
                              </div>
                              ...
                              <p className="text-xs mt-1 italic text-muted-foreground">
                                {order.orderItems?.length || 0} items • {shipping.city || 'No City'}
                              </p>
                            </div>
                          ...
                          {isExpanded && (
                            <div className="mt-4 pt-4 border-t border-dashed border-gray-200 grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-top-2 duration-300">
                              {/* Left Column: Delivery Address */}
                              <div className="space-y-2 bg-gray-50/50 p-4 rounded-xl border border-gray-100">
                                <h4 className="font-bold text-sm text-gray-700 uppercase tracking-wider">📍 Delivery Address</h4>
                                <div className="text-sm space-y-1 text-gray-600">
                                  <p><span className="font-semibold text-gray-800">Customer Name:</span> {order.user?.name || 'Guest'}</p>
                                  {order.user?.email && <p><span className="font-semibold text-gray-800">Email:</span> {order.user.email}</p>}
                                  <p><span className="font-semibold text-gray-800">Phone Number:</span> {shipping.phone || 'N/A'}</p>
                                  <p><span className="font-semibold text-gray-800">Street Address:</span> {shipping.address || 'N/A'}</p>
                                  <p><span className="font-semibold text-gray-800">City / Town:</span> {shipping.city || 'N/A'}</p>
                                  <p><span className="font-semibold text-gray-800">Pincode:</span> {shipping.pincode || 'N/A'}</p>
                                  <p><span className="font-semibold text-gray-800">Payment Mode:</span> <span className="font-bold text-red-600">{order.paymentMethod || order.payment_method || 'COD'}</span></p>
                                </div>
                              </div>

                              {/* Right Column: Order Items */}
                              <div className="space-y-2">
                                <h4 className="font-bold text-sm text-gray-700 uppercase tracking-wider">🛍️ Ordered Items</h4>
                                  ...
                                  {order.orderItems?.map((item: any, idx: number) => (
                                    <div key={idx} className="flex gap-3 items-center border-b border-gray-100 pb-2 last:border-0">
                                      <img 
                                        src={item.image} 
                                        alt={item.name} 
                                        className="w-10 h-12 object-cover rounded bg-gray-50 border"
                                        onError={(e) => {
                                          e.currentTarget.src = '/placeholder.jpg'
                                        }}
                                      />
                                      <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-gray-500">Qty: {item.qty || item.quantity || 1} × ₹{(item.price || 0).toLocaleString()}</p>
                                      </div>
